  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className={cn(
          "flex lg:max-w-[85%] w-full fixed lg:top-5 inset-x-0 mx-auto border lg:rounded-3xl rounded-b-3xl shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)] z-[5000] px-10 py-5 items-center lg:justify-evenly justify-between space-x-9 border-white/[0.2] bg-zinc-50 dark:bg-zinc-950",
          className
        )}
      >
        ...
        <div className="hidden lg:flex items-center lg:gap-x-5">
          {navItems.slice(0, 4).map((navItem: any, idx: number) => (
            <Link
              ...
              href={navItem.link}
              className={cn(
                `${isActive(navItem.link)} relative dark:text-neutral-50 items-center flex text-black dark:hover:text-neutral-300 hover:text-neutral-500 transition-all`
              )}
            >
              <span className="block sm:hidden">{navItem.icon}</span>
              <span className="hidden sm:block text-lg font-thin uppercase tracking-wider">
                {navItem.name}
              </span>
            </Link>
          ))}
