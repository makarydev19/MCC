  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className={cn(
          "flex lg:max-w-[75%] w-full fixed lg:top-5 inset-x-0 mx-auto border rounded-xl shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)] z-[5000] px-10 py-5 items-center lg:justify-center justify-between space-x-9 border-white/[0.2] bg-zinc-100 dark:bg-zinc-950",
        ...
        <div className="hidden lg:flex items-center lg:gap-x-5">
          {navItems.slice(0, 4).map((navItem: any, idx: number) => (
            <Link
              ...
              href={navItem.link}
              className={cn(
                `${isActive(navItem.link)} relative dark:text-neutral-50 items-center flex space-x-2 text-black dark:hover:text-neutral-300 hover:text-neutral-500`
              )}
            >
